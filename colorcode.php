...
Plugin URI: http://www.pizn.net/03-11-2012/colorcode
Description: Javascript 代码高亮插件，为您的代码提供多种配色方案，支持html，css，javascript和xml语言。
Version: 1.01
Author: zhanxin lin
...
function wp_colorcode_style(){
	...
	if($wp_colorcode_themes=='random'){
		...
		$theme_style[5]='cc-lotus';
		$rand_theme=array_rand($theme_style);
		$wp_colorcode_themes=$theme_style[$rand_theme];
	}
	$wp_colorcode_css_url = plugins_url('/css/' . $wp_colorcode_themes . '.css?ver=20121102', __FILE__);
	if(file_exists(TEMPLATEPATH . "/cc-knight.css?ver=20111206")){
		$wp_colorcode_css_url = get_bloginfo("template_url") . "/cc-knight.css?ver=20121102";
	}
	if(get_option('wp_colorcode_themes')!=''){
		echo '<link rel="stylesheet" type="text/css" href="'.$wp_colorcode_css_url.'" media="screen" />' . "\n";
	}
	else{
		$wp_colorcode_css_url = plugins_url('/css/cc-knight.css?ver=20121102', __FILE__);
		echo '<link rel="stylesheet" type="text/css" href="'.$wp_colorcode_css_url.'" media="screen" />' . "\n";
	}
}
...
function wp_colorcode_js(){
	...
	$wp_colorcode_js_url = plugins_url('/js/wp-colorcode.js?ver=20121102', __FILE__);
?>
	<!--colorcode_start-->
